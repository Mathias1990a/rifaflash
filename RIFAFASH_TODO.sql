-- ============================================
-- RIFAFASH - SQL COMPLETO ÚNICO
-- Ejecutar esto UNA SOLA VEZ en Supabase
-- ============================================
...
RETURNS VOID AS $$
BEGIN
  UPDATE rooms SET occupied_count = occupied_count + 1 WHERE id = room_id_param;
END;
$$ LANGUAGE plpgsql;
...
DROP POLICY IF EXISTS "Allow all users insert" ON users;
DROP POLICY IF EXISTS "Allow all users update" ON users;
DROP POLICY IF EXISTS "Allow all numbers operations" ON numbers;
...
INSERT INTO users (full_name, dni, phone, cvu_alias, referral_code)
VALUES ('Administrador', 'admin', '0000000000', 'admin', 'ADMIN001')
ON CONFLICT (dni) DO NOTHING;

-- 7. VERIFICACIÓN
SELECT 'Configuración completada' as estado;
SELECT room_id, COUNT(*) as total_numeros FROM numbers GROUP BY room_id ORDER BY room_id;
